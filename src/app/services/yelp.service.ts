...
export class YelpService {
    ...
    // 💥 RESTORED: This helper assigns colors & emojis based on cuisine
    ...
    async getRestaurants(lat: number, lng: number, radiusMiles: number): Promise<Restaurant[]> {
        const headers = new HttpHeaders({
            Authorization: `Bearer ${this.apiKey}`,
            accept: 'application/json'
        });

        // Yelp requires meters. Max is 40,000 (~25 miles)
        const radiusMeters = Math.min(Math.floor(radiusMiles * 1609.34), 40000);

        const url = `${this.baseUrl}?latitude=${lat}&longitude=${lng}&radius=${radiusMeters}&limit=20&term=restaurants`;

        const response: any = await firstValueFrom(this.http.get(url, { headers }));

        return response.businesses.map((b: any) => {
            // 💥 FIXED: We have to calculate 'hasReservations' before we can use it!
            const hasReservations = b.transactions && b.transactions.includes('restaurant_reservation');

            // Generate our dynamic colors
            ...
            return {
                ...
                takesReservations: hasReservations, // This will now work perfectly
                emoji: visuals.emoji,               // Colors restored!
                color: visuals.color,
                bg: visuals.bg
            };
        });
